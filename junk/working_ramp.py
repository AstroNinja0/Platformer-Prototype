    ...
    def move(self, movement, tiles):
        ...
        for ramp in ramps:
            ...
            if self.rect.colliderect(hitbox): # check if player collided with the bounding box for the ramp
                ...
                target_x = hitbox.x + tile_size - pos_width

                if self.pos[0] < target_x and ramp.ramp == 2 and movement[0] < 0:
                    self.rect.right = hitbox.left
                    collision_types['right'] = True
                elif self.pos[0] > target_x and ramp.ramp == 1:
...
# generate test map
tiles = [tile([6, 7], 'red', 1), tile([5, 8], 'red', 1), tile([6, 8], 'red'), tile([4, 6], 'red'), tile([4, 5], 'red', 2), tile([3, 5], 'red')]
for i in range(10):
